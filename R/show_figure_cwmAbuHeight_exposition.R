...
sites <- read_csv2("data_processed_sites.csv", col_names = T, na = "na", col_types = 
  ...
  select(cwmAbuHeight, surveyYear, constructionYear, id, block, location, plot, exposition, plotAge, PC1, PC2, PC3, changeType, ffh, conf.low, conf.high) %>%
...
pdata <- ggemmeans(m2, terms = c("exposition"), type = "fe")
data2 <- rename(sites, predicted = cwmAbuHeight, x = exposition, group = surveyYearF)
meandata <- filter(pdata, x == "south")
pd <- position_dodge(.6)
(graph <- ggplot(pdata, 
    ...
    scale_y_continuous(limits = c(.1, .8), breaks = seq(-100, 100, .1)) +
    #scale_shape_manual(values = c(1,16)) +
    labs(x = "FFH type", y = expression(paste("CWM of Height"))) +
    themeMB()
)

### Save ###
setwd(here("outputs/figures"))
ggsave("figure_cwmHeight_exposition_(800dpi_8x7cm).tiff",
