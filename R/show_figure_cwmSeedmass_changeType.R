...
sites <- read_csv2("data_processed_sites.csv", col_names = T, na = "na", col_types = 
  ...
  select(cwmAbuSeedmass, surveyYear, constructionYear, id, block, location, plot, exposition, plotAge, PC1, PC2, PC3, changeType, ffh, conf.low, conf.high) %>%
...
data2 <- rename(sites, predicted = cwmAbuSeedmass, x = changeType, group = surveyYearF)
...
(graph <- ggplot(pdata, 
    ...
    #facet_grid(~ group) +
    annotate("text", label = "n.s.", x = 4.2, y = 4) +
    scale_y_continuous(limits = c(0, 4), breaks = seq(-100, 100, .5)) +
    #scale_shape_manual(values = c(1,16)) +
    labs(x = "FFH type", y = expression(paste("CWM of Seedmass"))) +
    themeMB()
)

### Save ###
setwd(here("outputs/figures"))
ggsave("figure_cwmSeedmass_(800dpi_8x7cm).tiff",
