...
sites <- read_csv2("data_processed_sites.csv", col_names = T, na = "na", col_types = 
  ...
  select(fdisAbuSla, fdisAbuSeedmass, fdisAbuHeight, surveyYear, constructionYear, id, block, location, plot, exposition, plotAge, PC1, PC2, PC3, changeType, ffh, conf.low, conf.high) %>%
...
pdata <- ggemmeans(m2, terms = c("changeType", "type"), type = "fe")
data2 <- rename(sites, predicted = n, x = changeType, group = type)
...
(graph <- ggplot(pdata, 
    ...
    facet_grid(~ group) +
    annotate("text", label = "n.s.", x = 4.2, y = 1.1) +
    scale_y_continuous(limits = c(0, 1.1), breaks = seq(-100, 100, .2)) +
...
ggsave("figure_fdisSingle_(800dpi_8x7cm).tiff",
