...
```{r posteriors}
combined <- rbind(
  mcmc_intervals_data(posterior1, prob = 0.66, prob_outer = 0.95),
  mcmc_intervals_data(posterior2, prob = 0.66, prob_outer = 0.95)
  )
combined$model <- rep(
  c("Model 1", "Model 2"),
  each = ncol(posterior1)
  )
pos <- position_nudge(y = ifelse(combined$model == "Model 2", 0, 0.1))
ggplot(data = combined, aes(x = m, y = parameter, color = model)) + 
