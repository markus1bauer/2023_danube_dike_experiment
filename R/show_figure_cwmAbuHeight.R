...
sites <- read_csv2("data_processed_sites.csv", col_names = T, na = c("na", "NA"), col_types = 
  ...
  select(cwmAbuHeight, id, surveyYear, constructionYear, plotAge, locationAbb, block, plot, side, exposition, PC1, PC2, PC3, changeType, ffh) %>%
...
dwplot(m,
  ...
  xlab(expression(Delta~CWM~canopy~height~"["*m*"]")) +
  annotate("text", label = expression(paste("R"^2*""[m]*"= 0.05")), x = .2, y = 5.3, hjust = T, size = 2) +
  annotate("text", label = expression(paste("R"^2*""[m2]*"= 0.43")), x = .2, y = 4.9, hjust = T, size = 2) +
  annotate("text", label = expression(paste("R"^2*""[c]*"= 0.43")), x = .2, y = 4.5, hjust = T, size = 2) +
  themeMB()

### Save ###
setwd(here("outputs/figures"))
ggsave("figure_cwmAbuHeight_(800dpi_16x5cm).tiff",
