...
ggplot(sites, aes(x = factor(surveyYear), y = n)) + 
  ...
  facet_grid( ~ block) # Plot for figure
#3way
...
ggplot(sites %>% filter(surveyYear == 2021),
       aes(x = targetType, y = n, color = seedDensity)) + 
  geom_quasirandom(alpha = 0.5, dodge.width = 0.8) +
  geom_boxplot(fill = "transparent") +
  facet_wrap(~ exposition)
#4way
...
ggplot(data = sites,
       aes(x = surveyYear_fac, y = n, color = sandRatio)) + 
  geom_quasirandom(alpha = 0.5, dodge.width = 0.8) +
  geom_boxplot(fill = "transparent") +
  facet_grid(exposition ~ substrateDepth)
ggplot(data = sites,
       aes(x = surveyYear_fac, y = n, color = seedDensity)) + 
  geom_quasirandom(alpha = 0.5, dodge.width = 0.8) +
  geom_boxplot(fill = "transparent") +
  facet_grid(exposition ~ targetType)
ggplot(data = sites,
...
m_simple <- brm(n ~ targetType + exposition + sandRatio + surveyYear_fac +
                  ...
                  (1 | block/exposition/substrateDepth/sandRatio/plot) +
                  (1 | botanist_year),
                data = sites, 
...
m_full <- brm(n ~ (targetType + exposition + sandRatio + surveyYear_fac +
                ...
                targetType:seedDensity:exposition:surveyYear_fac +
...
m4 <- lmerTest::lmer(n ~ (exposition + targetType + sandRatio +
                            seedDensity + substrateDepth +
                       surveyYear_fac)^3 +
                       (1 | block_exposition/substrateDepth/sandRatio/plot) +
                       (1 | botanist_year),
                     data = sites,
...
(emm <- emmeans(m4, revpairwise ~ targetType + sandRatio |
                  exposition | surveyYear_fac, type = "response"))
(emm <- emmeans(m4, revpairwise ~ substrateDepth + sandRatio |
                  exposition | surveyYear_fac, type = "response"))
(emm <- emmeans(m4, revpairwise ~ seedDensity + targetType |
                  exposition | surveyYear_fac, type = "response"))
