...
All sown seed mixtures can be found on GitHub in the folder \`outputs/tables/table_a2_seed_mixtures.csv\`. There it is indicated which species are combined and the ratio in the specific seed mixture.

# Appendix A3

The community-weighted means (CWM) of the seed mixtures used for the field experiment: specific leaf area (SLA), seed mass and canopy height. The CWMs were weighted by seed weight in the seed mixture and indicate differences between the target types hay meadows and dry calcareous grasslands. The functional traits of specific leaf area (SLA), seed mass and canopy height (available traits of all species: 99.2%, 98.8%, 99.6%) were retrieved from the databases LEDA [@Kleyer.2008] and TRY [@Kattge.2020].
...
read_csv(
  here("outputs", "tables", "table_a4_specieslist.csv"),
  col_names = TRUE,
  na = c("na", "NA"),
  col_types = cols(.default = "?", name = "f")
  ) %>%
  ...
  bold(part = "header") %>%
  style(j = 1, pr_t = fp_text_default(italic = TRUE)) %>%
  bg(j = 2, bg = scales::col_numeric(palette = c("white", "grey60"), domain = c(0, 931))) %>%
  autofit()
```
